...
function startPosition(e) {
    ...
    const advancedBrushes = ['smudge', 'blur-brush', 'sharpen-brush', 'dodge', 'burn'];
...
function drawAirbrush() {
    ...
    const colorRgb = hexToRgb(brushColor);
...
function draw(e) {
    ...
    if (currentTool === 'brush' || currentTool === 'eraser' || currentTool === 'clone' || advancedBrushes.includes(currentTool)) {
        ...
        if (currentTool === 'clone') {
            ...
            const colorRgb = hexToRgb(brushColor); // For eraser this is ignored by destination-out usually, or we use black
            ...
            // Store last position specifically for brush interpolation
            window.lastX = localX;
            window.lastY = localY;
        }
        ...
        // Advanced Brushes (Smudge, Blur, Sharpen, Dodge, Burn)
            // Need to read pixels around the brush, process them, and write back.
            const r = Math.ceil(brushSize / 2);

            // For smudge, we might need to sample from further back if moving fast.
            // Calculate a combined bounding box that includes the previous position
...
function setTool(tool) {
    ...
    const map = {
        ...
        'rect': 9, 'circle': 10, 'line': 11, 'text': 12, 'marquee': 13, 'lasso': 14, 'poly-lasso': 15
    };
